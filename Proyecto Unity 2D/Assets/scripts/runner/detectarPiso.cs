...
public class detectarPiso : MonoBehaviour {
	...
	void Start () {
		//personaje.rigidbody2D.velocity = new Vector2(2*velocidad,0);
		//personaje.rigidbody2D.AddForce(new Vector2(1 * velocidad,0));
	}
	
	// Update is called once per frame
	void Update () {
		personaje.transform.Translate(velocidad * Time.deltaTime,0,0);
		...
		if (Input.GetMouseButtonDown(0))
		{
			...
			personaje.rigidbody2D.AddForce(new Vector2(5,200));
			tocaPiso = false;
